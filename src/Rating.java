...
public class Rating 
{
	...
	public static int rateMaterial()
	{
		...
		if(bishopCounter>=2) //bishops are better in pairs
		...
		for(int i=0; i<64; i++)
		{
			switch(AlphaBetaChess.chessBoard[i/8][i%8])
			{
				...
				case "Q": counter += queenBoard[i/8][i%8];
					break;
				case "A": if(material>=1750) {counter += kingMidBoard[i/8][i%8]; counter += AlphaBetaChess.possibleA(AlphaBetaChess.kingPositionC).length()*10;}
						else {counter += kingEndBoard[i/8][i%8]; counter += AlphaBetaChess.possibleA(AlphaBetaChess.kingPositionC).length()*30;}
					break;
			}
		}
		return counter;
